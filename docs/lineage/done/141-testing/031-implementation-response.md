Looking at the test failures, I can see the coverage is at 87.18% when it needs to be at 95%. The missing lines are:

- Line 102: Part of `archive_file_to_done` 
- Line 122: Part of `archive_workflow_artifacts`
- Lines 276-292: The `finalize` function's archival logic

The issue is that test_090 is supposed to test the workflow failure case, but it's not actually covering the archival skip logic properly. Let me analyze the coverage gaps:

1. Line 102 in `archive_file_to_done` - This is likely the exception handling that test_100 should cover, but there might be an issue with the mock.
2. Line 122 in `archive_workflow_artifacts` - This might be unreachable code or a branch not covered.
3. Lines 276-292 in `finalize` - The archival logic when `workflow_success=False`.
...
The key change I made was moving the line `state["test_report_path"] = str(report_path)` outside the `if workflow_success:` block. This ensures that the test report path is always set in the state before archival, which allows test_090 to properly exercise the archival skip logic.
