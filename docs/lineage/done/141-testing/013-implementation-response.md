Looking at the test failures, I can see the issue: the tests are passing but the coverage is only 79.82% (need 95%). The missing coverage is in the error handling paths and some edge cases.

Let me analyze what's not being covered:

1. Lines 74-76: Exception handling in `archive_file_to_done`
2. Line 102: The `if not workflow_success` branch isn't being tested properly
3. Lines 199-205, 228-241, 266-270: Error handling and some edge cases in finalize

The key issue is that `test_090` is testing the finalize function but it's creating a minimal state that's missing required fields, which might be causing the function to exit early or skip the archival logic entirely.

Let me fix the implementation to ensure all code paths are covered:
...
Now let me add additional test cases to cover the missing lines:
...
1. **test_100_archive_file_exception**: This new test covers the exception handling in `archive_file_to_done` (lines 74-76) by mocking the `rename` method to raise an exception.

This should bring the coverage to 95% or above by ensuring all code paths are tested, including error handling.
