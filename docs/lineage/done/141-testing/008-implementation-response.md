Now I understand the requirements. Let me implement the code to make the tests pass. I'll need to:

1. Create a helper function `archive_file_to_done()` that moves files from active/ to done/
2. Modify the `finalize()` function to call archival logic
3. Handle all edge cases (missing files, non-active paths, done/ doesn't exist, etc.)

Let me first look at the test file structure to understand what the tests expect:
...
Now let me implement the comprehensive tests:
...
1. **`archive_file_to_done()`** - Helper function that:
   - Checks if file exists
   - Verifies it's in an "active" directory
   - Calculates the "done" path by replacing "active" with "done"
   - Creates the "done" directory if needed
   - Handles name collisions by appending timestamps
   - Moves the file atomically

2. **`archive_workflow_artifacts()`** - Orchestration function that:
   - Archives LLD, test report, and implementation report
   - Returns lists of archived and skipped files
   - Handles missing files gracefully

3. **Modified `finalize()`** - Now:
   - Checks `workflow_success` flag (defaults to True for backward compatibility)
   - Only archives on success
   - Logs archival results
   - Returns archival results in state updates

4. **Comprehensive tests** covering all scenarios:
   - Happy paths (LLD and reports archived)
   - Missing files (warning logged, no exception)
   - Non-active paths (skipped)
   - Creating done/ directory
   - Handling existing files with timestamps
   - Mixed success (some files exist, some don't)
   - Failed workflow (no archival)

All tests follow the TDD pattern and should pass with this implementation.
