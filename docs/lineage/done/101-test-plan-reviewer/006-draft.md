...
4. Clear PASS/REVISE/ERROR verdict
...
- **Markdown Sanitization:** Strip HTML tags, script elements, and executable content before processing using `dompurify` library (MIT licensed)
...
- `dompurify` library used for sanitization (MIT license, verified compliant)
...
- `src/lib/markdown-sanitizer.ts` — HTML/script stripping utility using `dompurify`
...
- `dompurify` npm package for markdown sanitization (MIT license)
...
- [ ] Markdown sanitizer strips HTML/script tags before processing using `dompurify`
...
- [ ] Unit tests written and passing for parser, sanitizer, and extractor
