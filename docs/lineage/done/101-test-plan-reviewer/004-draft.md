...
- **Markdown Sanitization:** Strip HTML tags, script elements, and executable content before processing
- **Token Management:** Pre-flight token count check with graceful truncation strategy
...
- `src/lib/markdown-sanitizer.ts` — HTML/script stripping utility
...
- [ ] Markdown sanitizer strips HTML/script tags before processing
- [ ] Token counter with truncation logic implemented
