...
- `hooks/prepare-commit-msg-tdd.sh` — Prepare-commit-msg hook to append TDD footer to commit messages
- `tools/tdd-gate.py` — Main TDD enforcement CLI tool (uses subprocess list args, not shell=True)
- `tools/tdd-audit.py` — Audit trail generation and reporting
- `tools/tdd-pending-issues.py` — Async pending issue creation processor
...
- `husky` or `pre-commit` framework (optional, for managed hook installation)
...
- [ ] Exit code 5 error message suggests checking file naming conventions
...
- [ ] Async pending issue processor implemented
...
- [ ] `tools/tdd-pending-issues.py` documented
...
tdd-gate --verify-red  # Should fail: "Invalid red phase: no tests found (exit code 5). Did you name your file test_*.py?"
