...
3. System runs tests and verifies they fail (red phase) with exit code 1
...
2. System runs tests and detects exit code is not `1` (test failure)
3. System distinguishes between:
   ...
   - Exit code `5`: No tests collected (invalid - tests not found)
...
3. System authenticates via `gh` CLI (uses existing `GITHUB_TOKEN` from environment)
4. System logs override with reason and timestamp to audit trail
5. PR is flagged for follow-up test coverage
6. Technical debt issue is auto-created via `gh issue create`
...
4. Red phase proof stored in commit message footer: `TDD-Red-Phase: <sha>:<timestamp>`
...
2. CI extracts and validates footers from commit history on the branch
3. No `.tdd-state.json` committed to avoid merge conflicts
4. Local `.tdd-state.json` is git-ignored for developer convenience only
5. Audit log at `docs/reports/{IssueID}/tdd-audit.md` is strictly additive (append-only)
...
- **CI Gate:** Extract `TDD-Red-Phase` footer from commit history; fail if missing for new test files
...
- Local state files (`.tdd-state.json`) excluded from git and sensitive data scans
...
- [ ] Red phase proof is written to commit message footer: `TDD-Red-Phase: <sha>:<timestamp>`
- [ ] CI gate extracts and validates `TDD-Red-Phase` footer from commit history
...
- [ ] Override creates follow-up issue via `gh issue create` for test coverage debt
- [ ] Audit trail appends to `docs/reports/{IssueID}/tdd-audit.md`
- [ ] Works with pytest: `test_*.py` pattern detection, exit code validation
- [ ] Works with Jest: `*.test.js` and `*.spec.js` pattern detection, exit code validation
...
- [ ] CI integration documented with commit footer extraction
...
# Verify: override logged, follow-up issue created via gh CLI
...
### Verify CI Extraction
```bash
# Simulate CI footer extraction
