...
def is_sensitive_file(file_path: Path) -> bool:
    ...
    Matching is case-insensitive for directory components and checks exact
    matches, suffix matches, and prefix matches for filenames.
    ...
    for pattern in SENSITIVE_PATTERNS:
        ...
        if pattern_lower.startswith("."):
            ...
            if name.endswith(pattern_lower):
                return True
            # Prefix match for dotfiles (e.g., ".env.local" matches ".env")
...
def read_file_with_budget(
    ...
    try:
        ...
        if "\x00" in content:
            logger.warning("Binary file detected (null bytes): %s", file_path)
            return empty_result
...
Two bugs fixed from the previous implementation:

1. **`is_sensitive_file` prefix matching** — Added `startswith` check for dot-prefixed patterns so `.env.local` and `.env.production` are correctly detected as sensitive (previously only checked exact match and `endswith`)

2. **Null-byte binary detection** — Added `"\x00" in content` check after reading file text to catch binary files like `.gif` that have valid UTF-8 headers but contain null bytes (previously these would pass through as valid text content)
