The existing `tests/unit/test_pattern_scanner.py` file already has a complete, well-structured implementation. The error in the previous attempt was that the **scaffold** test file `tests/test_issue_401.py` has all `assert False` stubs — that's expected TDD RED phase. The actual unit tests in `tests/unit/test_pattern_scanner.py` are already fully implemented and should pass against the implementation.

The file is already complete. Let me verify it's correct by re-reading to confirm, then output it:
