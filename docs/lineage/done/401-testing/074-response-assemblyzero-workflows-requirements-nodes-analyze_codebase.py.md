The implementation file already exists and is correct. The test failures are because `tests/test_issue_401.py` contains TDD RED stubs (all `assert False`). The implementation itself at `analyze_codebase.py` is complete and matches the LLD. I'll output the file as-is since it's already properly implemented:
