I notice that I only have Playwright browser tools available. Let me check if I can use other approaches. Based on the task requirements, I need to implement the `get_checkpoint_db_path()` function and related functionality. Let me work with what's provided in the LLD and test file.

Based on the LLD specification, I need to:

1. Modify `src/agentos/workflow/checkpoint.py` - Add `get_checkpoint_db_path()` with per-repo logic
2. Update `.gitignore` - Add `.agentos/` pattern
3. Update `docs/workflow.md` - Document the new behavior
...
Now the test file with full implementations:
...
Finally, the `.gitignore` update:

```gitignore
# File: .gitignore (append these lines)

# AgentOS per-repo workflow database
.agentos/
```
...
```markdown
# File: docs/workflow.md (relevant section to add/update)

## Workflow Database Location

The workflow checkpoint database stores state for issue workflows. By default, it uses a **per-repository** location for isolation.

### Database Path Resolution

The database path is determined in the following priority order:

1. **Environment Variable**: If `AGENTOS_WORKFLOW_DB` is set to a non-empty value, that path is used. Tilde (`~`) expansion is supported.
   ```bash
   export AGENTOS_WORKFLOW_DB=~/custom/workflow.db
   ```

2. **Per-Repository**: If running inside a git repository, the database is stored at:
   ```
   {repo_root}/.agentos/issue_workflow.db
   ```

3. **Fail Closed**: If not in a git repository and no environment variable is set, the command exits with an error (exit code 1).

### Benefits of Per-Repo Storage

- **Concurrent Workflows**: Multiple repositories can run workflows simultaneously without database conflicts
- **Worktree Support**: Each git worktree gets its own isolated database
- **Clean Separation**: Workflow state stays with the project it belongs to

### Migration from Global Database

If you previously used the global database at `~/.agentos/issue_workflow.db`, your existing data remains untouched. To continue using the global location, set the environment variable:

```bash
export AGENTOS_WORKFLOW_DB=~/.agentos/issue_workflow.db
```

### .gitignore

The `.agentos/` directory is automatically added to the project's `.gitignore` patterns. This directory should not be committed to version control.
```