...
Updated By: Issue #101 LLD creation
Update Reason: Fixed mechanical validation errors - reordered files so directories appear before their contents
Previous: Initial LLD draft for Test Plan Reviewer skill
-->
...
|----|----------|------|-------|-----------------|---------------|
| 010 | Happy path - test plan passes | Auto | Valid test plan + issue | PASS verdict | All ACs covered |
| 020 | Missing AC coverage | Auto | Test plan missing 2 ACs | REVISE verdict | Gap analysis shows missing ACs |
| 030 | Secrets detected - API key | Auto | Test plan with `API_KEY=sk-xxx` | BLOCKED verdict | Line number reported |
| 040 | Secrets detected - password | Auto | Test plan with `password: "secret"` | BLOCKED verdict | Line number reported |
| 050 | Token limit exceeded | Auto | 100KB test plan | Truncated + warning | Review completes with warning |
| 060 | API failure (mock) | Auto | Force retry exhaustion | ERROR verdict | 3 retries logged |
| 070 | Dry run mode | Auto | `--dry-run` flag | Payload output, no API call | No API request made |
| 080 | Mock mode | Auto | `TEST_PLAN_REVIEW_MOCK=true` | Fixture-based response | No API request made |
| 090 | XSS in test plan | Auto | `<script>alert('x')</script>` | Script stripped | Sanitized content clean |
| 100 | Empty test plan | Auto | Empty markdown file | REVISE verdict | "No test cases found" |
| 110 | Issue not found | Auto | Non-existent issue number | ERROR verdict | Graceful error message |
| 120 | Audit trail completeness | Auto | Any successful review | Complete audit file | Hash, timestamp, verdict present |
