The existing implementation looks correct. The test failures are all from TDD stub tests (`assert False`), not from implementation bugs. The implementation itself is complete and well-structured. Let me output it as-is:
