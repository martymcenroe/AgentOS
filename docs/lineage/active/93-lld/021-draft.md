...
## 1. Context & Goal
* **Issue:** #93
* **Objective:** Create a proactive research workflow that searches GitHub/Web for solutions to a given problem, analyzes top implementations, compares them against internal code, and produces an "Innovation Brief" documenting specific deficiencies.
* **Status:** Draft
...
| `tools/run_scout_workflow.py` | CLI entry point handling args, privacy warnings, path safety, observability, and output generation. |
...
| `langsmith` | Observability and tracing for workflow execution (Reviewer Requirement). |
...
ESTIMATED_EXTERNAL_TOKENS = 15000 # Buffer for search results
```
...
def validate_safe_input_path(path: str) -> str:
    """
    Ensures input path exists and is readable.
    Does NOT enforce confinement (input can be anywhere in project).
    Raises ValueError if invalid.
    """
    pass

def resolve_secure_output_path(topic: str, custom_path: Optional[str]) -> str:
    """
    Resolves final output path.
    ENFORCES CONFINEMENT: Path must resolve to be inside the project root.
    If no custom_path: Sanitizes topic to create filename in ideas/active/.
    Checks for file existence to prevent accidental overwrite (raises FileExistsError).
    """
    pass
...
from agentos.workflows.scout.token_estimator import check_budget_preflight, BudgetExceededError

def main():
    setup_logging()
    setup_observability() # LangSmith
    ...
    # 2. Output Path Security Check
    try:
        output_path = resolve_secure_output_path(args.topic, args.output)
    except (ValueError, FileExistsError) as e:
        console.print(f"[red]Path Error: {e}[/red]")
        sys.exit(1)

    # 3. Privacy Gate (User Confirmation)
    if internal_content and not confirm_transmission(args.internal, args.yes):
        console.print("[yellow]Aborted by user.[/yellow]")
        sys.exit(0)

    # 4. Cost Control Pre-flight
    try:
    ...
    with console.status("[bold green]Running Scout Workflow..."):
    ...
    if args.format == 'json':
        ...
        console.print(f"[bold green]Brief saved to {output_path}[/bold green]")
```
    ...
    for repo_meta in state['found_repos']:
        ...
        try:
            ...
            truncated_readme = readme[:MAX_CHARS_PER_FILE]
             ...
             truncated_readme = "No README found."
...
*   **Safety Handling:**
    *   **Input:** Path validation ensures read access.
    *   **Output:** `resolve_secure_output_path` uses `os.path.abspath` and `os.path.commonpath` to ensure the output writes effectively confine to the project directory (blocking `../../etc/passwd` attacks).
...
*   **Observability:** `langsmith` integration enabled via environment variables for tracing steps, combined with `rich` for CLI user feedback.
...
7.  **Output Safety:** Strict worktree confinement for output files. Fail if file exists.
8.  **Output Formatting:** Generate Markdown (default) or JSON.
9.  **Observability:** Trace execution via LangSmith if configured.
...
### 5.2 Data Pipeline
```ascii
[CLI] -> [Secure Path Check] -> [Budget Check] -> [Privacy Check]
                                     |
                                     v
                            [LangGraph Execution] (Traced by LangSmith)
                                     |
...
*   **Env Vars:** `GITHUB_TOKEN`, `GEMINI_API_KEY`, `LANGSMITH_API_KEY`.
...
sequenceDiagram
    ...
    CLI->>CLI: Validate Paths (Confinement)
    ...
    CLI->>Graph: Invoke(State) [Start Trace]
...
| **Path Traversal (Input)** | CLI verifies file exists/readable. |
| **Path Traversal (Output)** | `resolve_secure_output_path` enforces confinement to project root using `os.path.commonpath`. Rejects paths containing `..` that escape root. |
...
| 4 | Output Path Traversal | Unit | `--output ../../etc/passwd` | Error | Path security error. |
| 5 | Output Exists | E2E | Topic="Existing" | Error | "File exists" error. |
...
### Code
- [ ] `run_scout_workflow.py` implemented with path confinement and privacy gates.
