...
```python
def main():
    args = parse_args() # topic, internal, yes, max_tokens, format
    ...
    if args.internal:
        try:
            ...
            print_error(f"Error reading internal target: {e}")
            sys.exit(1)
    ...
    if args.format == "json":
        ...
        # Prevent overwrite logic (append timestamp if exists)
        ...
        print_success(f"Brief generated: {filename}")
```

**Graph Logic (Gap Analyst Node)**
```python
def node_gap_analyst(state):
    repos_context = format_repos_for_llm(state['analyzed_repos'])
    internal_code = state.get('internal_file_content')
    
    if internal_code:
        prompt = PROMPT_GAP_ANALYSIS.format(
            topic=state['topic'],
            external_context=repos_context,
    ...
    return {"gap_analysis_result": response.text}
```

### 2.6 Technical Approach
*   **Orchestration:** `LangGraph` manages the linear flow. This allows future extension (e.g., adding a "Search Refinement" loop if results are poor).
...
*   **Token Budget:** Estimation uses `tiktoken` (cheap, local) as a proxy for Gemini tokens. A buffer (15k tokens) is reserved for external search results to prevent mid-stream failure.

## 3. Requirements
1.  **Search:** Use GitHub API to find repos > 500 stars matching the topic.
...
| **AsyncIO implementation** | Faster concurrent requests. | Adds complexity to CLI tool structure. | **Accepted** - Use `asyncio` for the Node Extractor to fetch 3 repos in parallel. |
...
| **API Key Exposure** | Keys are read from ENV. Not logged. Not included in generated briefs. |
...
|--------|--------|----------|
| **Total Runtime** | < 45 seconds | Parallel fetch for GitHub content (3 concurrent). 5s timeout on searches. |
...
|------|--------|------------|------------|
| **No Repos Found** | Empty Report | Medium | Fallback: Suggest broader search terms in output. Minimum star count drops to 100 on retry (future). |
