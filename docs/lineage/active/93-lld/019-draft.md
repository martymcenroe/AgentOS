...
def resolve_output_path(topic: str, custom_path: Optional[str]) -> str:
    ...
    - If custom_path provided: validates directory exists, and file is writable.
...
def main():
    ...
    # 2. Privacy Gate (User Confirmation)
    ...
    try:
        ...
        console.print(f"[red]Error: Output file '{args.output or 'generated path'}' already exists. Please move it or specify different --output.[/red]")
        sys.exit(1)
...
*   **Safety Handling:**
    ...
    *   **Output:** `resolve_output_path` validates that user-provided paths are writeable and checks for existing files to prevent accidental overwrite.
...
*   **Observability:** `rich` console status for UI feedback and standard `logging` for debug/error traces in the background.
...
9.  **Observability:** Log errors and display progress status to CLI user.
...
| **Path Traversal (Output)** | Output filename sanitization; valid directory check; prevent writing to critical system paths. |
