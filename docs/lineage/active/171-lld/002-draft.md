...
- [ ] Should the 50% threshold be configurable via workflow state or hardcoded?
- [ ] Should we differentiate between "file replaced" (new content, old deleted) vs "file heavily modified" (incremental changes)?
- [ ] What constitutes "explicit approval" - a specific keyword, interactive prompt, or both?
...
2. Run `git diff --stat --staged` to get overview
3. FOR each changed file:
   a. Get line count before (git show HEAD:file | wc -l)
...
| Threshold configuration | Hardcoded, Config file, CLI flag | Hardcoded with constant | Start simple; can add config in future issue |
...
|---------|------------|--------|
| Command injection via filenames | Sanitize all file paths before passing to subprocess | TODO |
...
| Auto-mode bypass | Explicit check and fail if --auto detected | TODO |
| False negative (missing dangerous change) | Show full diff for any flagged file, not just stats | Addressed |
| Workflow state corruption | Atomic state updates, clear approval status on error | TODO |
...
| 050 | Approval continues workflow | Auto | "APPROVE" input | review_approved=True | State updated, no error |
| 060 | Rejection halts workflow | Auto | "REJECT" input | review_approved=False | Workflow halted cleanly |
