# LLD-101: Unified Governance Workflow
...
Create `run_governance_workflow.py` that:
...
**File:** `agentos/workflows/governance/config.py`
...
**File:** `agentos/workflows/governance/state.py`
...
**File:** `agentos/workflows/governance/graph.py`

```python
from typing import Literal
from langgraph.graph import END, StateGraph

from .state import GovernanceWorkflowState
from .nodes import (
    load_input,
    generate_draft,
    human_gate_draft,
...
def route_after_load(state: GovernanceWorkflowState) -> Literal["N1_draft", "end"]:
    """Route after input loading."""
    if state.get("error_message"):
        return "end"
    return "N1_draft"

def route_after_draft(state: GovernanceWorkflowState) -> Literal["N2_gate_draft", "N3_review", "end"]:
...
def route_after_draft_gate(state: GovernanceWorkflowState) -> Literal["N3_review", "N1_draft", "end"]:
...
def route_after_review(state: GovernanceWorkflowState) -> Literal["N4_gate_verdict", "N5_finalize", "N1_draft", "end"]:
    """Route after review."""
    if state.get("error_message"):
        return "end"
...
def route_after_verdict_gate(state: GovernanceWorkflowState) -> Literal["N5_finalize", "N1_draft", "end"]:
...
def build_governance_workflow() -> StateGraph:
    ...
    Graph structure:
        ...
        N5_finalize
            ...
            END
    """
    workflow = StateGraph(GovernanceWorkflowState)
...
```bash
python tools/run_governance_workflow.py [options]
...
| `agentos/workflows/governance/__init__.py` | ~10 | Package init |
| `agentos/workflows/governance/config.py` | ~80 | Workflow configuration |
| `agentos/workflows/governance/state.py` | ~100 | Unified state |
| `agentos/workflows/governance/graph.py` | ~150 | Parameterized StateGraph |
| `agentos/workflows/governance/audit.py` | ~400 | Unified audit utilities |
| `agentos/workflows/governance/nodes/__init__.py` | ~20 | Node exports |
| `agentos/workflows/governance/nodes/load_input.py` | ~150 | Input loading |
| `agentos/workflows/governance/nodes/generate_draft.py` | ~180 | Draft generation |
| `agentos/workflows/governance/nodes/human_gate.py` | ~200 | Human gates |
| `agentos/workflows/governance/nodes/review.py` | ~100 | LLM review |
| `agentos/workflows/governance/nodes/finalize.py` | ~200 | Finalization |
| `tools/run_governance_workflow.py` | ~400 | CLI runner |
| **Total** | **~2240** | |
...
pytest tests/unit/test_governance_config.py -v

# Test state transitions
pytest tests/unit/test_governance_state.py -v
```
...
python tools/run_governance_workflow.py \
...
python tools/run_governance_workflow.py --type issue --select
...
│   ├── test_governance_config.py      # Workflow configuration
│   ├── test_governance_state.py       # State validation
│   ├── test_governance_audit.py       # Audit trail functions
...
```bash
python tools/run_governance_workflow.py --type lld --issue 42 --debug
```
...
```bash
python tools/run_governance_workflow.py --type lld --issue 42 --perf-log
```
...
```bash
python tools/run_governance_workflow.py --type lld --issue 42 --dry-run
```
...
```bash
python tools/run_governance_workflow.py --type lld --issue 42 --verify-paths
```
...
```bash
# Must run successfully with real API calls
python tools/run_governance_workflow.py --type lld --issue {real_issue_number}
...
```mermaid
sequenceDiagram
    participant CLI as run_governance_workflow.py
    participant Config as WorkflowConfig
    participant Graph as GovernanceWorkflow
    ...
    CLI->>Graph: build_governance_workflow()
