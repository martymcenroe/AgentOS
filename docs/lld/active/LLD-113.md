...
- **Configurability**: Consider making the `persist_directory` configurable via an environment variable (e.g., `ASSEMBLYZERO_VECTOR_STORE_PATH`) to facilitate testing or different deployment environments, defaulting to `.assemblyzero/vector_store/` if unset.
