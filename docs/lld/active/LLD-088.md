...
- [x] ~~Should the similarity threshold (0.7) be configurable via environment variable or CLI flag?~~ **RESOLVED: Yes. Use environment variable `ASSEMBLYZERO_RAG_THRESHOLD` with default 0.7.**
...
class LibrarianConfig(TypedDict):
    similarity_threshold: float  # Default: 0.7, env: ASSEMBLYZERO_RAG_THRESHOLD
    top_k_candidates: int        # Default: 5
    top_k_results: int           # Default: 3
    embedding_model: str         # Default: "all-MiniLM-L6-v2", env: ASSEMBLYZERO_RAG_MODEL
...
def get_config() -> LibrarianConfig:
    ...
    Environment Variables:
        ASSEMBLYZERO_RAG_THRESHOLD: Similarity threshold (default: 0.7)
        ASSEMBLYZERO_RAG_MODEL: Embedding model name (default: "all-MiniLM-L6-v2")
...
- [ ] Environment variable configuration implemented (ASSEMBLYZERO_RAG_THRESHOLD, ASSEMBLYZERO_RAG_MODEL)
...
| G1.3 | "Add ASSEMBLYZERO_RAG_THRESHOLD and ASSEMBLYZERO_RAG_MODEL to the LibrarianConfig definition" | YES - Added to Section 2.3 and 2.4 |
