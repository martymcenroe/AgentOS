...
| 170 | Mock LLM mode | Auto | ASSEMBLYZERO_MOCK_LLM=1 | No API calls | Fixtures used |
...
ASSEMBLYZERO_MOCK_LLM=1 poetry run pytest tests/workflows/implementation/ -v
...
ASSEMBLYZERO_MOCK_LLM=1 poetry run pytest tests/workflows/implementation/test_audit_logging.py -v

# Run graph routing tests
ASSEMBLYZERO_MOCK_LLM=1 poetry run pytest tests/workflows/implementation/test_graph.py -v

# Run with coverage
ASSEMBLYZERO_MOCK_LLM=1 poetry run pytest tests/workflows/implementation/ -v --cov=assemblyzero/workflows/implementation
```
