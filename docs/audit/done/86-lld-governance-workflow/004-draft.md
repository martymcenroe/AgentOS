...
## Labels
`workflow`, `langgraph`, `governance`

## Effort Estimate
**T-shirt Size: L** — Complex state management with checkpointing, conditional routing, and VS Code integration.

## Budget Estimate

### Worst-Case Cost Analysis
- **Model:** Gemini 3 Pro (governance review node)
- **Estimated Input Tokens per Review:** ~8,000 tokens (LLD content + governance prompt)
- **Estimated Output Tokens per Review:** ~2,000 tokens (verdict + critique)
- **Maximum Iterations:** 5

| Scenario | Reviews | Input Tokens | Output Tokens | Est. Cost* |
|----------|---------|--------------|---------------|------------|
| Happy Path | 1 | 8,000 | 2,000 | ~$0.08 |
| Typical (2 iterations) | 2 | 16,000 | 4,000 | ~$0.16 |
| Worst Case (5 iterations) | 5 | 40,000 | 10,000 | ~$0.40 |

*Based on Gemini 3 Pro pricing at $0.00125/1K input, $0.005/1K output

### Cost Control Strategy
1. **Hard iteration cap:** Workflow exits after 5 iterations with guidance message
2. **Checkpoint persistence:** Failed runs don't restart from scratch, reducing redundant API calls
3. **Designer node runs once:** Only governance review loops; initial generation is not repeated
4. **Local-first edits:** Human refinement happens offline before each API call
...
### Scenario 5: Dry Run / Mock Mode
1. Developer runs `python tools/run_lld_workflow.py --issue 42 --mock`
2. System loads fixture data from `tests/fixtures/lld_workflow/`
3. System executes full state graph without GitHub or Gemini API calls
4. System logs state transitions and node outputs to console
5. Result: Developer validates workflow logic without incurring API costs
...
### Mock / Dry-Run Mode
1. CLI supports `--mock` flag for offline development and testing
2. Mock mode loads fixtures from `tests/fixtures/lld_workflow/`:
   - `issue_42.json` — Mock GitHub issue response
   - `designer_output.md` — Mock LLD draft from designer node
   - `governance_approved.json` — Mock approval response
   - `governance_rejected.json` — Mock rejection with critique
3. Mock mode skips VS Code blocking (`code --wait`) and auto-continues
4. Mock mode logs all state transitions to stdout for debugging
5. Real API calls are completely bypassed in mock mode
...
5. Optional argument: `--mock` (use fixtures instead of live APIs)
6. Exit code 0 on approval, non-zero on failure
...
- **Mock Infrastructure:** Fixture-based testing allows full graph validation without API dependencies
...
- Mock mode uses only local fixtures, no credentials required
...
- `agentos/workflows/lld/mock.py` — Mock implementations for offline testing
- `tools/run_lld_workflow.py` — CLI entry point
- `docs/LLDs/active/.gitkeep` — Ensure output directory exists
- `tests/fixtures/lld_workflow/issue_42.json` — Mock issue fixture
- `tests/fixtures/lld_workflow/designer_output.md` — Mock designer output
- `tests/fixtures/lld_workflow/governance_approved.json` — Mock approval
- `tests/fixtures/lld_workflow/governance_rejected.json` — Mock rejection
...
- **Diff view between iterations** — future UX enhancement
...
- [ ] `--mock` flag executes full workflow using local fixtures without API calls
...
- [ ] Mock mode with fixture loading implemented
- [ ] Unit tests for state transitions (using mock mode)
...
- [ ] Document mock mode usage for development
...
1. Run `python tools/run_lld_workflow.py --issue 42 --mock`
2. Verify state transitions logged to console
3. Verify workflow completes using fixture data
4. Modify fixtures to test rejection loop behavior
5. Use for CI/CD pipeline integration tests
