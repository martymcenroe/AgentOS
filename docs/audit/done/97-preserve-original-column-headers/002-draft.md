...
## Objective
Update CSV generation to use original PDF column headers verbatim instead of programmer-friendly canonical names.
...
3. Result: No normalization or sanitization applied to header text
...
### Output Format
1. First row of CSV contains original headers
...
- **CSV writer:** Use `table.original_headers` for `writerow()` instead of canonical names
...
- `src/output/csv_writer.py` — Use original headers in output generation
...
- [ ] CSV writer uses original headers
...
### Verification
- [ ] Output CSV headers match PDF headers exactly
