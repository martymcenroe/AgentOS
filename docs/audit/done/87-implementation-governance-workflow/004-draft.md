...
6. Pytest subprocess calls MUST include a 300-second (5 minute) timeout to prevent hanging tests from freezing the agent
...
- **Test Arbiter:** Python `subprocess.run(['pytest', '-v', '--tb=short'], timeout=300)` captures real output. No LLM interpretation of pass/fail. Timeout prevents hanging tests.
...
- **Path Validator:** Resolves all paths via `pathlib.Path.resolve()`, verifies they start with `cwd`, rejects traversal attempts.
...
- `agentos/workflows/implementation/nodes/test_gates.py` — N2 (must fail) and N4 (must pass) nodes with timeout
...
- `agentos/workflows/implementation/path_validator.py` — Centralized path security validation
...
## Effort Estimate
**Large (L)** — Complex state management, subprocess handling, security validation, and mock infrastructure
