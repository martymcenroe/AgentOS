...
## User Story
As a Governance Lead, I want to verify the auto-routing logic to ensure the system fails closed, preserves verdict history, and routes exhausted issues to manual triage.

## Objective
Verify that the auto-routing workflow correctly handles clean verdicts, accumulates verdict history across revisions, and fails closed after exceeding the maximum revision cycle limit.
...
No sensitive data involved. This is a workflow verification task using only non-sensitive issue metadata. No elevated permissions required.
